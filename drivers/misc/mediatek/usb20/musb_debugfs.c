...
#if !IS_ENABLED(CONFIG_PHY_MTK_TPHY) && !IS_ENABLED(CONFIG_MTK_MUSB_DUAL_ROLE)
	/* Refer to mtk_phy_procfs_init() or musb_dr_debugfs_init() */
...
void musb_dr_debugfs_init(struct musb *musb)
{
	...
	/* Refer to mtk_phy_procfs_init() or musb_init_debugfs() */
	proc_mkdir(PROC_DIR_MTK_USB, NULL);
#endif
