...
struct mtk_panel_params {
	...
	bool use_icom_power_on_seq;
	bool use_icom_power_off_seq;
#endif
	unsigned int change_fps_by_vfp_send_cmd;
#ifdef CONFIG_DRM_MTK_ICOM_SKIP_FRAME
	unsigned int skip_frame_base_fps;
#endif
	unsigned int pll_clk;
	unsigned int data_rate;
...
#ifdef CONFIG_DRM_MTK_ICOM_DSI_POWER_SEQUENCE
	int (*panel_lp_enable)(struct drm_panel *panel);
	int (*panel_lp_disable)(struct drm_panel *panel);
#endif
#ifdef CONFIG_DRM_MTK_ICOM_LCM_SET_DISPLAY_ON_DELAY
	int (*display_on)(struct drm_panel *panel,
		void *dsi_drv, dcs_write_gce cb, void *handle);
	void (*display_on_get_state)(struct drm_panel *panel, bool *state);
#endif
