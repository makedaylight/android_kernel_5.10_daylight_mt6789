...
int mtk_drm_crtc_set_display_on(struct drm_crtc *crtc)
{
	struct mtk_drm_crtc *mtk_crtc = to_mtk_crtc(crtc);
	struct mtk_ddp_comp *comp = mtk_ddp_comp_request_output(mtk_crtc);
	struct cmdq_pkt *cmdq_handle;
	...
	comp->funcs->io_cmd(comp, NULL, DSI_DISP_ON_GET_STATE, &state);
	if (state)
		return 0;
...
#define MAX_SKIP_COMMIT_PLANE_CNT (1)
	/* Screen data will be prepared in the second drm_atomic_helper_commit_planes() */
	if (commit_plane_cnt < MAX_SKIP_COMMIT_PLANE_CNT) {
		commit_plane_cnt++;
		return 0;
	}
	...
	/* DSC would use VM CMD in  DSI VDO mode only */
	client = (is_frame_mode) ? mtk_crtc->gce_obj.client[CLIENT_CFG] :
				mtk_crtc->gce_obj.client[CLIENT_DSI_CFG];
	cmdq_handle =
		cmdq_pkt_create(client);
	...
	comp->funcs->io_cmd(comp, cmdq_handle, DSI_SET_DISP_ON, NULL);
...
#ifndef CONFIG_DRM_MTK_ICOM_LCM_SET_DISPLAY_ON_DELAY
	/* 12. enable ESD check */
...
#ifdef CONFIG_DRM_MTK_ICOM_LCM_SET_DISPLAY_ON_DELAY
	/* display on */
	mtk_drm_crtc_set_display_on(crtc);
#endif
