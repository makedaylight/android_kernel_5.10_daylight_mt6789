...
#define ENABLE_DSI_VDO_TIMING_LOG 0
#define ENABLE_DSI_PHY_TIMING_LOG 0
...
#if ENABLE_DSI_PHY_TIMING_LOG
	pr_info("[DISP-DSI] DPHY_TIMCONFIG: DataRate=%u CycleTime=%u(ns) UnitInterval=%u(ns) lane#=%u\n",
		dsi->data_rate, cycle_time, ui, dsi->lanes);
	pr_info("[DISP-DSI] DPHY_TIMCONFIG: HS_TRAIL=%u HS_ZERO=%u HS_PRPR=%u LPX=%u TA_GET=%u TA_SURE=%u TA_GO=%u CLK_TRAIL=%u CLK_ZERO=%u CLK_HS_PRPR=%u\n",
		hs_trail, hs_zero, hs_prpr, lpx, ta_get, ta_sure, ta_go, clk_trail, clk_zero, clk_hs_prpr);
	pr_info("[DISP-DSI] CLK_HS_POST=%u CLK_HS_EXIT=%u CONT_DET=%u DA_HS_EXIT=%u DA_HS_SYNC=%u\n",
		clk_hs_post, clk_hs_exit, cont_det, da_hs_exit, da_hs_sync);
#endif

	dsi->data_phy_cycle = lpx + da_hs_exit + hs_prpr + hs_zero + 2;
#if ENABLE_DSI_PHY_TIMING_LOG
	pr_info("[DISP-DSI] DPHY_TIMCONFIG: data_phy_cycle=%u\n", dsi->data_phy_cycle);
#endif
...
#if ENABLE_DSI_PHY_TIMING_LOG
{
	u32 timcon0 = readl(dsi->regs + DSI_PHY_TIMECON0);
	u32 timcon1 = readl(dsi->regs + DSI_PHY_TIMECON1);
	u32 timcon2 = readl(dsi->regs + DSI_PHY_TIMECON2);
	u32 timcon3 = readl(dsi->regs + DSI_PHY_TIMECON3);

	pr_info("[DISP-DSI] DSI_PHY_TIMECON0=0x%08x, DSI_PHY_TIMECON1=0x%08x, DSI_PHY_TIMECON2=0x%08x, DSI_PHY_TIMECON3=0x%08x\n",
...
#if ENABLE_DSI_VDO_TIMING_LOG
	pr_info("[DISP-DSI] %s: PS_WC=%u VACT_NL=%u DSI_SIZE_CON=0x%x DSI_PSCTRL=0x%x\n",
		__func__, ps_wc, height, size, val);
#endif
}
...
#if ENABLE_DSI_VDO_TIMING_LOG
	pr_info("[DISP-DSI] VDO_TIMCONFIG_DSC: VSA_NL=%u VBP_NL=%u VFP_NL=%u VACT_NL=%u\n",
		readl(dsi->regs + DSI_VSA_NL), readl(dsi->regs + DSI_VBP_NL),
		readl(dsi->regs + DSI_VFP_NL), readl(dsi->regs + DSI_VACT_NL));
	pr_info("[DISP-DSI] VDO_TIMCONFIG_DSC: HSA_WC=%u HBP_WC=%u HFP_WC=%u BLLP_WC=%u\n",
		readl(dsi->regs + DSI_HSA_WC), readl(dsi->regs + DSI_HBP_WC),
		readl(dsi->regs + DSI_HFP_WC), readl(dsi->regs + DSI_BLLP_WC));
#endif
}

static void DSI_PHY_CLK_LP_PerLine_config(struct mtk_dsi *dsi)
{
	if (dsi->mode_flags & MIPI_DSI_MODE_VIDEO) {

		unsigned int timcon0; // LPX
		unsigned int timcon2; // CLK_HS_TRAIL, CLK_HS_ZERO
		unsigned int timcon3; // CLK_HS_EXIT, CLK_HS_POST, CLK_HS_PREP
		unsigned int hsa;
		unsigned int hbp;
		unsigned int hfp;
		unsigned int bllp;
		unsigned int ps;
		unsigned int hstx_ckl_wc = 0;
		unsigned int v_a, v_b, v_c;

#define DIFF_CLK_LANE_LP 0x10

		hsa			= readl(dsi->regs + DSI_HSA_WC);
		hbp			= readl(dsi->regs + DSI_HBP_WC);
		hfp			= readl(dsi->regs + DSI_HFP_WC);
		bllp		= readl(dsi->regs + DSI_BLLP_WC);
		ps			= readl(dsi->regs + DSI_PSCTRL);
		hstx_ckl_wc	= readl(dsi->regs + DSI_HSTX_CKL_WC);
		timcon0		= readl(dsi->regs + DSI_PHY_TIMECON0);
		timcon2		= readl(dsi->regs + DSI_PHY_TIMECON2);
		timcon3		= readl(dsi->regs + DSI_PHY_TIMECON3);
		...
		if (dsi->mode_flags & MIPI_DSI_MODE_VIDEO_BURST) {
			...
			v_a = REG_FLD_VAL_GET(DSI_HBP_WC_FLD_DSI_HBP_WC, hbp)
				+ REG_FLD_VAL_GET(HFP_WC_FLD_REG_DSI_HFP_WC, hfp)
				+ REG_FLD_VAL_GET(DSI_PS_WC, ps)
				+ REG_FLD_VAL_GET(DSI_BLLP_WC_FLD_DSI_BLLP_WC, bllp)
				+ 32;
			v_b = (REG_FLD_VAL_GET(FLD_CLK_HS_EXIT, timcon3)
				+ REG_FLD_VAL_GET(FLD_CLK_HS_PREP, timcon3)
				+ REG_FLD_VAL_GET(FLD_CLK_HS_ZERO, timcon2))
				* dsi->lanes;
			v_c = (REG_FLD_VAL_GET(FLD_CLK_HS_POST, timcon3)
				+ REG_FLD_VAL_GET(FLD_CLK_HS_TRAIL, timcon2))
				* dsi->lanes;

			DDPDUMP("%s, v_a - v_b = 0x%x, HSTX_CKLP_WC = 0x%x\n", __func__, (v_a - v_b), hstx_ckl_wc);
			DDPDUMP("%s, v_b + v_c = 0x%x, HFP_WC = 0x%x\n", __func__, (v_b + v_c), hfp);
			DDPDUMP("%s, Will Reconfig in order to fulfill LP clock lane per line\n", __func__);

			// B+C < HFP ,here diff is 0x10
			writel((v_b + v_c + DIFF_CLK_LANE_LP), (dsi->regs + DSI_HFP_WC));

			if (dsi->mode_flags & MIPI_DSI_CLOCK_NON_CONTINUOUS) {
				unsigned int new_hfp;
				unsigned int new_hstx_ckl_wc = 0;

				new_hfp = readl(dsi->regs + DSI_HFP_WC);

				// v_a = hbp.HBP_WC + new_hfp.HFP_WC + ps.DSI_PS_WC + bllp.BLLP_WC + 32
				v_a = REG_FLD_VAL_GET(DSI_HBP_WC_FLD_DSI_HBP_WC, hbp)
					+ REG_FLD_VAL_GET(HFP_WC_FLD_REG_DSI_HFP_WC, new_hfp)
					+ REG_FLD_VAL_GET(DSI_PS_WC, ps)
					+ REG_FLD_VAL_GET(DSI_BLLP_WC_FLD_DSI_BLLP_WC, bllp)
					+ 32;

				new_hstx_ckl_wc = v_a - v_b;

				writel(new_hstx_ckl_wc, dsi->regs + DSI_HSTX_CKL_WC);

				DDPDUMP("%s, new HSTX_CKL_WC = 0x%x, HFP_WC = 0x%x\n", __func__, new_hstx_ckl_wc,
							REG_FLD_VAL_GET(HFP_WC_FLD_REG_DSI_HFP_WC, new_hfp));
			}
		}
		/*
		 ...
		 */
			...
			v_a = REG_FLD_VAL_GET(DSI_HSA_WC_FLD_DSI_HSA_WC, hsa)
				...
				+ 32;
			v_b = (REG_FLD_VAL_GET(FLD_CLK_HS_EXIT, timcon3)
				+ REG_FLD_VAL_GET(FLD_LPX, timcon0)
				+ REG_FLD_VAL_GET(FLD_CLK_HS_PREP, timcon3)
				+ REG_FLD_VAL_GET(FLD_CLK_HS_ZERO, timcon2))
				* dsi->lanes;
			v_c = (REG_FLD_VAL_GET(FLD_CLK_HS_POST, timcon3)
...
#if ENABLE_DSI_VDO_TIMING_LOG
	pr_info("[DISP-DSI] VDO_TIMCONFIG: VSA_NL=%u VBP_NL=%u VFP_NL=%u VACT_NL=%u\n",
		readl(dsi->regs + DSI_VSA_NL), readl(dsi->regs + DSI_VBP_NL),
		readl(dsi->regs + DSI_VFP_NL), readl(dsi->regs + DSI_VACT_NL));
	pr_info("[DISP-DSI] VDO_TIMCONFIG: HSA_WC=%u HBP_WC=%u HFP_WC=%u BLLP_WC=%u\n",
		readl(dsi->regs + DSI_HSA_WC), readl(dsi->regs + DSI_HBP_WC),
		readl(dsi->regs + DSI_HFP_WC), 0);
#endif
...
#if ENABLE_DSI_VDO_TIMING_LOG
		pr_info("[DISP-DSI] VDO_TIMCONFIG: LP HFP_WC=0x%08x (HFP=%u; hs_vb_ps_wc=0x%x)\n",
			readl(dsi->regs + DSI_HFP_WC), dsi->hfp_byte, hs_vb_ps_wc);
#endif
	} else if (dsi->ext && dsi->ext->params->lp_perline_en) {
		/*
		 * Refer to LK
		 * vendor/mediatek/proprietary/bootable/bootloader/lk2/platform/mediatek/mt6789/disp/ddp_dsi.c
		 * void DSI_PHY_CLK_LP_PerLine_config(enum DISP_MODULE_ENUM module, void *cmdq, LCM_DSI_PARAMS *dsi_params)
		 */
		DSI_PHY_CLK_LP_PerLine_config(dsi);
	}
}
	...
	int force_lcm_update)
{
	...
	/* Prepare (Power On) the panel before MIPI dsi clk output is enabled to prevent the power leakage */
	if (dsi->panel && ext && ext->params && ext->params->use_icom_power_on_seq) {
...
#ifdef CONFIG_DRM_MTK_ICOM_DSI_POWER_SEQUENCE
		if (ext && ext->params && ext->params->use_icom_power_on_seq
			&& ext->funcs && ext->funcs->panel_lp_enable) {
			if (((!dsi->doze_enabled && !dsi->pending_switch) || force_lcm_update)
				&& ext->funcs->panel_lp_enable(dsi->panel)) {
				DDPPR_ERR("[LP] failed to enable the panel\n");
				return;
			}
		} else if ((!ext || !ext->params || !ext->params->use_icom_power_on_seq)
			&& ((!dsi->doze_enabled && !dsi->pending_switch) || force_lcm_update)
			&& drm_panel_prepare(dsi->panel)) {
			DDPPR_ERR("failed to prepare the panel\n");
			return;
		}
				   ...
				   int force_lcm_update, bool need_wait)
{
	...
	if (!mtk_dsi_is_cmd_mode(&dsi->ddp_comp)) {
#if defined(CONFIG_DRM_MTK_ICOM_STOP_VDO_MODE_CMDQ_TIMEOUT_WORKAROUND) && defined(CONFIG_DRM_MTK_ICOM_DSI_VFP_MODE)
		if (!cmdq_handle)
			mtk_crtc_pkt_create((struct cmdq_pkt **)&cmdq_handle, &mtk_crtc->base, mtk_crtc->gce_obj.client[CLIENT_CFG]);
		mtk_ddp_comp_io_cmd(&dsi->ddp_comp, cmdq_handle, DSI_VFP_MAX_VREFRESH_MODE, NULL);
#endif
...
#ifdef CONFIG_DRM_MTK_ICOM_DSI_POWER_SEQUENCE
			if (dsi->ext && dsi->ext->params && dsi->ext->params->use_icom_power_off_seq
				&& dsi->ext->funcs && dsi->ext->funcs->panel_lp_disable) {
				if (dsi->ext->funcs->panel_lp_disable(dsi->panel)) {
					DDPPR_ERR("[LP] failed to disable the panel\n");
				}
			} else if ((!dsi->ext || !dsi->ext->params || !dsi->ext->params->use_icom_power_off_seq)
...
#ifdef CONFIG_DRM_MTK_ICOM_DSI_POWER_SEQUENCE
	/* Unprepare (Power Off) the panel after MIPI dsi clk output is disabled to prevent the power leakage */
	if (dsi->panel && dsi->ext && dsi->ext->params && dsi->ext->params->use_icom_power_off_seq) {
		if ((!new_doze_state && !skip_panel_switch) || force_lcm_update) {
			if (drm_panel_unprepare(dsi->panel))
				DRM_ERROR("failed to unprepare the panel\n");
		}
...
#ifdef CONFIG_DRM_MTK_ICOM_SKIP_FRAME
		else if ((cur_panel_params->change_fps_by_vfp_send_cmd ||
				adjust_panel_params->change_fps_by_vfp_send_cmd) &&
				(drm_mode_vrefresh(old_mode) != drm_mode_vrefresh(adjust_mode)))
			mode_chg_index |= MODE_DSI_VFP;
#endif
...
#ifdef CONFIG_DRM_MTK_ICOM_SKIP_FRAME
	if (ext->params->skip_frame_base_fps > 0)
		vrefresh = ext->params->skip_frame_base_fps;
#endif
			  ...
			  enum mtk_ddp_io_cmd cmd, void *params)
{
	...
	case CONNECTOR_IS_ENABLE:
		...
		*enable = dsi->output_en;
		break;
#ifdef CONFIG_DRM_MTK_ICOM_DSI_VFP_MODE
	case DSI_VFP_PREFERRED_MODE:
	{
		unsigned int vfront_porch = 0;
		unsigned int preferred_vfront_porch = 0;
		struct drm_display_mode *preferred_mode, *next;
		...
		list_for_each_entry_safe(preferred_mode, next, &dsi->conn.modes, head) {
			if (preferred_mode->type & DRM_MODE_TYPE_PREFERRED) {
				preferred_vfront_porch = preferred_mode->vsync_start - preferred_mode->vdisplay;
			}
		}

		DDPMSG("vfront_porch=%d , preferred_vfront_porch=%d\n", vfront_porch, preferred_vfront_porch);
		...
		break;
	case DSI_VFP_MAX_VREFRESH_MODE:
	{
		unsigned int vfront_porch = 0;
		unsigned int max_mode_vrefresh = 0;
		unsigned int max_mode_vfront_porch = 0;
		...
		list_for_each_entry_safe(max_mode, next, &dsi->conn.modes, head) {
			if (drm_mode_vrefresh(max_mode) > max_mode_vrefresh) {
				max_mode_vrefresh = drm_mode_vrefresh(max_mode);
				max_mode_vfront_porch = max_mode->vsync_start - max_mode->vdisplay;
			}
		}

		DDPMSG("vfront_porch=%d , max_mode_vfront_porch=%d\n", vfront_porch, max_mode_vfront_porch);
	...
	case DSI_DISP_ON_GET_STATE:
	{
		panel_ext = mtk_dsi_get_panel_ext(comp);
		if (!(panel_ext && panel_ext->funcs &&
			panel_ext->funcs->display_on_get_state))
			break;

		panel_ext->funcs->display_on_get_state(dsi->panel, (bool *)params);
		break;
	}
	case DSI_SET_DISP_ON:
	{
		struct mtk_dsi *dsi =
			container_of(comp, struct mtk_dsi, ddp_comp);
		...
		panel_ext = mtk_dsi_get_panel_ext(comp);
		if (panel_ext && panel_ext->funcs
			&& panel_ext->funcs->display_on)
			panel_ext->funcs->display_on(dsi->panel, dsi,
