		...
		struct mmc_command *cmd)
{
	...
	if (events & MSDC_INT_ACMDRDY) {
		...
		if (host->need_tune == TUNE_AUTOK_PASS) {
			...
			host->retune_times = 0;
			// pr_info("%s: msdc%d: %s retune_times=0\n", __func__, host->id, host->id ? "SD" : "eMMC");
		}
	} else {
		msdc_reset_hw(host);
		if (events & MSDC_INT_ACMDCRCERR) {
			...
			// pr_info("%s: msdc%d: %s TUNE_CMD_CRC\n", __func__, host->id, host->id ? "SD" : "eMMC");
		} else if (events & MSDC_INT_ACMDTMO) {
			cmd->error = -ETIMEDOUT;
			host->error |= REQ_STOP_TMO;
			host->need_tune = TUNE_CMD_TMO;
			// pr_info("%s: msdc%d: %s TUNE_CMD_TMO\n", __func__, host->id, host->id ? "SD" : "eMMC");
		}
		dev_err(host->dev,
	...
	if (check_data || (stop && stop->error)) {
		...
		if ((events & MSDC_INT_XFER_COMPL) && (!stop || !stop->error)) {
			...
			if (mrq && mrq->data && mrq->data->flags & MMC_DATA_WRITE) {
				...
				// pr_info("%s: msdc%d: %s TUNE_DATA_WRITE\n", __func__, host->id, host->id ? "SD" : "eMMC");
			} else if (data->flags & MMC_DATA_WRITE) {
				host->need_tune = TUNE_DATA_WRITE;
				// pr_info("%s: msdc%d: %s TUNE_DATA_WRITE\n", __func__, host->id, host->id ? "SD" : "eMMC");
			} else {
				host->need_tune = TUNE_DATA_READ;
				// pr_info("%s: msdc%d: %s TUNE_DATA_READ\n", __func__, host->id, host->id ? "SD" : "eMMC");
			}
...
static void msdc_set_initial_signal_voltage(struct msdc_host *host)
{
	...
	if (!msdc_set_signal_voltage(mmc, MMC_SIGNAL_VOLTAGE_330))
		dev_dbg(mmc_dev(mmc), "initial signal voltage of 3.3v\n");
	else if (!msdc_set_signal_voltage(mmc, MMC_SIGNAL_VOLTAGE_180))
		dev_dbg(mmc_dev(mmc), "initial signal voltage of 1.8v\n");
	else if (!msdc_set_signal_voltage(mmc, MMC_SIGNAL_VOLTAGE_120))
		dev_dbg(mmc_dev(mmc), "initial signal voltage of 1.2v\n");
...
static void msdc_ops_set_ios(struct mmc_host *mmc, struct mmc_ios *ios)
{
	...
	case MMC_POWER_OFF:
		...
		if (host->pins_pull_down) {
			dev_dbg(host->dev, "%s: pins-pull-down\n", __func__);
		...
		if (!IS_ERR(mmc->supply.vqmmc) && host->vqmmc_enabled) {
			...
			if (host->id == MSDC_SD && host->sd_poweroff_reset_signal_volt)
...
static struct msdc_delay_phase get_best_delay(struct msdc_host *host, u32 delay)
{
	...
	dev_notice(host->dev, "phase: [map:%x] [start:%d] [maxlen:%d] [final:%d]\n",
		 delay, start_final, len_final, final_phase);
...
static int msdc_detect_bad_sd(struct msdc_host *host, u32 condition)
{
	...
	pr_notice("%s:bad_sd_detecter:%d (%d)\n", __func__, bad_sd_detecter[condition], ret);
...
static void msdc_sd_power_off(struct msdc_host *host)
{
	struct mmc_host *mmc = mmc_from_priv(host);
...
static int msdc_drv_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR(host->pins_default)) {
		...
		dev_err(&pdev->dev, "Cannot find pinctrl default: %d\n", ret);
		goto host_free;
	}
	pinctrl_select_state(host->pinctrl, host->pins_default);
#else
	...
	if (IS_ERR(host->pins_pull_down)) {
		...
		dev_info(&pdev->dev, "Cannot find pinctrl pull_down: %d\n", ret);
...
#endif
{
	...
	else if (host->id == MSDC_SD || host->id == MSDC_EMMC)
