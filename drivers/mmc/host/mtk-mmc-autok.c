	...
	if (host->id == MSDC_EMMC) {
		...
		case DATA_ERROR:
			...
			sdr_set_field(host->base + MSDC_PATCH_BIT,
					MSDC_PATCH_BIT_RD_DAT_SEL, 0);
			sdr_set_field(host->base + MSDC_IOCON,
				MSDC_IOCON_DSPL, orig_read_edge ^ 0x1);
			sdr_get_field(host->base + MSDC_IOCON,
				MSDC_IOCON_DSPL, &cur_read_edge);
...
int emmc_execute_autok(struct msdc_host *host, u32 opcode)
{
	...
	if (mmc->ios.timing == MMC_TIMING_MMC_HS400 ||
		...
		/* Using "do {} while();" because mmc_send_tuning() can not find the TUNE_DATA_WRITE error */
		i = 0;
		do {
			autok_err_type = -1;
			...
			for (j = 0; j < 3; j++) {
				...
				if (ret) {
					pr_notice("[AUTOK] eMMC mmc_send_tuning() err %d\n", ret);
					break;
				}
			}
		} while (ret && ++i < 3); /* max. limit must be odd */
	}
	...
	if (mmc->ios.timing == MMC_TIMING_UHS_SDR104 ||
		...
		do {
			...
			for (j = 0; j < 3; j++) {
				ret = mmc_send_tuning(mmc, opcode, NULL);
				if (ret) {
					pr_notice("[AUTOK] SD mmc_send_tuning() err %d\n", ret);
					break;
				}
			}
		} while (ret && ++i < 3); /* max. limit must be odd */
