...
static int gt9896s_spi_probe(struct spi_device *spi)
{
	...
	if (IS_ENABLED(CONFIG_OF) && spi->dev.of_node) {
		...
		if (gt9896s_find_touch_node == 1) {
            ...
            struct device_node *lcm_name = of_find_node_by_path("/chosen");
            if (lcm_name) {
                struct tag_videolfb *videolfb_tag = (struct tag_videolfb *)of_get_property(lcm_name, "atag,videolfb",(int *)&size);
                if (videolfb_tag) {
                    if (strcmp("ft8756_fhdp_dsi_vdo_auo_rt4801_drv", videolfb_tag->lcmname) == 0) {
                        gt9896s_config_buf = "6781v01_1080x2300";
                        gt9896s_firmware_buf = "6781v01_1080x2300";
                        ts_device->board_data.panel_max_x = 1080;
                        ts_device->board_data.input_max_x = 1080;
                        ts_device->board_data.panel_max_y = 2300;
                        ts_device->board_data.input_max_y = 2300;
                        ts_device->board_data.x2x = 0;
                        ts_device->board_data.y2y = 0;
                    } else if (strcmp("nt36672e_fhdp_dsi_vdo_120hz_jdi_dphy_hfp_lcm_drv", videolfb_tag->lcmname) == 0) {
                        gt9896s_firmware_buf = "6789v01";
                        gt9896s_config_buf = "6789v01";
                        ts_device->board_data.panel_max_x = 1080;
                        ts_device->board_data.input_max_x = 1080;
                        ts_device->board_data.panel_max_y = 2400;
                        ts_device->board_data.input_max_y = 2400;
                        ts_device->board_data.x2x = 1;
                        ts_device->board_data.y2y = 1;
                    } else {
						ts_info("%s, lcm not support", videolfb_tag->lcmname);
						r = -ENODEV;
						goto err_spi_buf;
                    }
                }
