...
#if defined(ILITEK_WAKELOCK_SUPPORT) && !defined(INOCO_I2C_WAKEUP_SOURCE)
...
#ifdef INOCO_PINCTRL_I2CPULL
static void ilitek_get_pinctrl(struct ilitek_ts_data *ts)
{
	tp_dbg("+%s()\n", __func__);

	ts->pinctrl = devm_pinctrl_get(ts->device);
	if (IS_ERR_OR_NULL(ts->pinctrl)) {
		if (IS_ERR(ts->pinctrl)) {
			tp_err("unable to get pinctrl, err: %ld\n", PTR_ERR(ts->pinctrl));
			ts->pinctrl = NULL;
		}
	} else {
		ts->active = pinctrl_lookup_state(ts->pinctrl, "active");
		if (IS_ERR(ts->active)) {
			tp_err("unable to lookup pinctrl state, err: %ld\n", PTR_ERR(ts->active));
			ts->active = NULL;
		}
		ts->sleep = pinctrl_lookup_state(ts->pinctrl, "sleep");
		if (IS_ERR(ts->sleep)) {
			tp_err("unable to lookup pinctrl state, err: %ld\n", PTR_ERR(ts->sleep));
			ts->sleep = NULL;
		}
...
static void ilitek_select_pinctrl(struct ilitek_ts_data *ts, bool status)
{
	int ret = 0;
	...
	if (ts->pinctrl == NULL) {
		tp_err("pinctrl is NULL can't select pinctrl\n");
		return;
	}

	if(status) {
		if (ts->active) {
			tp_dbg("select pinctrl to active\n");
			ret = pinctrl_select_state(ts->pinctrl, ts->active);
			if (ret)
				tp_err("unable to select pinctrl active: %d\n", ret);
		}
	} else {
		if (ts->sleep) {
			tp_dbg("select pinctrl to sleep\n");
			ret = pinctrl_select_state(ts->pinctrl, ts->sleep);
			if (ret)
				tp_err("unable to select pinctrl sleep: %d\n", ret);
		}
	}
...
#ifdef INOCO_ILITEK_MDELAY
	ilitek_mdelay(11); /* >10ms */
#else
	ilitek_mdelay(10); /* ~10ms */
#endif
	gpio_direction_output(ts->reset_gpio, 1);
	ilitek_mdelay(delay);
#endif
...
void __maybe_unused ilitek_gesture_handle(bool touch, int idx, int x, int y)
{
	struct input_dev *input = ts->input_dev;
#ifdef INOCO_KEY_WAKEUP
	static DEFINE_RATELIMIT_STATE(ratelimit, HZ/2, 1);
#endif
...
#ifdef INOCO_I2C_WAKEUP_SOURCE
	pm_wakeup_event(ts->device, 5000);
#endif
#if defined(ILITEK_WAKELOCK_SUPPORT) && !defined(INOCO_I2C_WAKEUP_SOURCE)
	...
	if (__ratelimit(&ratelimit))
		tp_dbg("tocuh wakeup event, report KEY_WAKEUP\n");
...
#ifdef INOCO_BOOT_UPDATE_SKIPPED_ILI_NOT_ERROR
		if (error == -EEXIST && !_strcasecmp(filename, "ilitek.ili"))
			error = 0;
#endif
		goto err_return;
	}
...
static int __maybe_unused ilitek_update_thread(void *arg)
{
	...
	/**
	 * 1. Can not use msleep() instead of mdelay()
	 * 2. Run ilitek_update_thread() later in order to remove this delay
	 */
#else
	ilitek_mdelay(100);
#endif
...
#ifndef INOCO_I2C_WAKEUP_SOURCE
...
#ifdef INOCO_PINCTRL_I2CPULL
		ilitek_select_pinctrl(ts, false);
#endif
	}
...
#ifdef INOCO_PINCTRL_I2CPULL
		ilitek_select_pinctrl(ts, true);
#endif
#ifdef INOCO_RESUME_NORESET
		if (ts->low_power_status != Low_Power_Sleep) {
			ilitek_reset(ts->dev->reset_time);
		}
#else
		...
		unsigned long event, void *data)
{
	...
	tp_msg("FB EVENT event: %lu\n", event);
	if (ev_data && event == ILITEK_EVENT_BLANK) {
		if (*ev_data == ILITEK_BLANK_POWERDOWN) {
			if (!ts->fb_blank_powerdown) {
				ilitek_suspend();
				ts->fb_blank_powerdown = true;
			}
		} else if (*ev_data == ILITEK_BLANK_UNBLANK || *ev_data == ILITEK_BLANK_NORMAL) {
			if (ts->fb_blank_powerdown) {
				ilitek_resume();
				ts->fb_blank_powerdown = false;
			}
		}
	}
...
static int ilitek_register_resume_suspend(void)
{
	...
	int error = 0;
#if ILITEK_PLAT != ILITEK_PLAT_MTK
#if defined(CONFIG_FB) || defined(CONFIG_QCOM_DRM) || IS_ENABLED(CONFIG_DRM_MEDIATEK)
...
#elif IS_ENABLED(CONFIG_DRM_MEDIATEK)
	error = mtk_disp_notifier_register("ilitek", &ts->fb_notif);
#else
...
#elif IS_ENABLED(CONFIG_DRM_MEDIATEK)
	mtk_disp_notifier_unregister(&ts->fb_notif);
#else
...
{
#ifdef INOCO_I2C_WAKEUP_SOURCE
	/* dts i2c "wakeup-source;", auto enable/disable wakeup */
	/* use pm_wakeup_event */
#else
	if (init) {
...
#ifdef INOCO_DEFAULT_RESET_DELAY
	ilitek_reset(INOCO_DEFAULT_RESET_DELAY);
#else
	ilitek_reset(1000);
#endif
...
#ifdef ILITEK_BOOT_UPDATE
#ifdef INOCO_REDUCE_UPDATE_THREAD_DELAY
	/* Run ilitek_update_thread() later in order to remove "mdelay(100);" in ilitek_update_thread() */
#else
...
#if defined(ILITEK_BOOT_UPDATE) && defined(INOCO_REDUCE_UPDATE_THREAD_DELAY)
	ts->update_thread = kthread_run(ilitek_update_thread, NULL,
				"ilitek_update_thread");
	if (IS_ERR(ts->update_thread))
		goto err_free_irq;
