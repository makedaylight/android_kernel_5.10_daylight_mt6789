...
#define debug_level(level, fmt, arg...)							\
	do {				       						\
		...
		if (level == ILITEK_ERR_LOG_LEVEL)					\
			pr_err("[ILITEK][E][%s:%d] "fmt, __func__, __LINE__, ##arg); 	\
		else if (level == ILITEK_INFO_LOG_LEVEL)				\
			pr_info("[ILITEK][I][%s:%d] "fmt, __func__, __LINE__, ##arg); 	\
		else if (level == ILITEK_DEBUG_LOG_LEVEL)				\
			pr_info("[ILITEK][D][%s:%d] "fmt, __func__, __LINE__, ##arg); 	\
	} while (0)
...
#if defined(CONFIG_FB) || IS_ENABLED(CONFIG_DRM_MEDIATEK)
