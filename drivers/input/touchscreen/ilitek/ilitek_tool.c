...
#ifdef INOCO_SHOW_FWV
static ssize_t ilitek_show_firmware_version(struct file *fp, char __user *buf,
					    size_t size, loff_t *off)
{
	int cnt;
	uint8_t str[256];
...
static struct PROC_FOPS_T ilitek_fops_show_fwversion = {
	.PROC_READ = ilitek_show_firmware_version,
};
#endif
...
#ifdef INOCO_SHOW_FWV
	if (!proc_create("show_fwversion", ILITEK_DEVICE_NODE_PERMISSON,
			 ilitek_proc_entry, &ilitek_fops_show_fwversion))
		tp_err("failed to create procfs show_fwversion.\n");
#endif
