...
static struct mt6789_mt6366_pinctrl {
	struct pinctrl *pinctrl;
	struct pinctrl_state *extamp_pullhigh;
	struct pinctrl_state *extamp_pulllow;
	int tmp;
} mt6789_mt6366_pinctrl;
...
static int mt6789_mt6366_pinctrl_event(struct snd_soc_dapm_widget *w,
				struct snd_kcontrol *kcontrol, int event)
{
	struct device *dev = w->dapm->card->dev;
	int ret = 0;

	dev_dbg(dev, "+%s()\n", __func__);

	dev_dbg(dev, "mt6789_mt6366_pinctrl: 0x%p\n", mt6789_mt6366_pinctrl);
	switch (event) {
	case SND_SOC_DAPM_PRE_PMU:
		if (mt6789_mt6366_pinctrl.extamp_pullhigh) {
			ret = pinctrl_select_state(mt6789_mt6366_pinctrl.pinctrl, mt6789_mt6366_pinctrl.extamp_pullhigh);
			if (ret) {
				dev_err(dev, "unable to select pinctrl extamp_pullhigh: %d\n", ret);
				return ret;
			}
			/* start-up time, AD51652: 4ms(max), LY8010BLT: 1ms(typ) */
			//usleep_range(2000, 2000); //TDB
			msleep(22);
		}
		break;
	case SND_SOC_DAPM_POST_PMD:
		if (mt6789_mt6366_pinctrl.extamp_pulllow) {
			ret = pinctrl_select_state(mt6789_mt6366_pinctrl.pinctrl, mt6789_mt6366_pinctrl.extamp_pulllow);
			if (ret) {
				dev_err(dev, "unable to select pinctrl extamp_pulllow: %d\n", ret);
				return ret;
			}
		}
		break;
...
static const struct snd_soc_dapm_widget mt6789_mt6366_widgets[] = {
	...
	SND_SOC_DAPM_SUPPLY("DAPM", SND_SOC_NOPM, 0, 0,
				mt6789_mt6366_pinctrl_event,
				SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
	SND_SOC_DAPM_REGULATOR_SUPPLY("extamp", 0, SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD), /* delay time TBD */
};
...
static const struct snd_soc_dapm_route mt6789_mt6366_routes[] = {
	...
	{"LINEOUT L", NULL, "extamp"},
	{"Headphone L Ext Spk Amp", NULL, "extamp"},
	{"Headphone R Ext Spk Amp", NULL, "extamp"},
	{"LINEOUT L", NULL, "DAPM"},
	{"Headphone L Ext Spk Amp", NULL, "DAPM"},
	{"Headphone R Ext Spk Amp", NULL, "DAPM"},
};
...
static int mt6789_mt6366_dev_probe(struct platform_device *pdev)
{
	...
	mt6789_mt6366_pinctrl.pinctrl = devm_pinctrl_get(&pdev->dev);
	if (IS_ERR_OR_NULL(mt6789_mt6366_pinctrl.pinctrl)) {
		if (IS_ERR(mt6789_mt6366_pinctrl.pinctrl)) {
			dev_err(&pdev->dev, "unable to get pinctrl, err: %ld\n", PTR_ERR(mt6789_mt6366_pinctrl.pinctrl));
			mt6789_mt6366_pinctrl.pinctrl = NULL;
		}
	} else {
		mt6789_mt6366_pinctrl.extamp_pullhigh = pinctrl_lookup_state(mt6789_mt6366_pinctrl.pinctrl, "extamp-pullhigh");
		if (IS_ERR(mt6789_mt6366_pinctrl.extamp_pullhigh)) {
			dev_err(&pdev->dev, "unable to lookup pinctrl state, err: %ld\n", PTR_ERR(mt6789_mt6366_pinctrl.extamp_pullhigh));
			mt6789_mt6366_pinctrl.extamp_pullhigh = NULL;
		}
		mt6789_mt6366_pinctrl.extamp_pulllow = pinctrl_lookup_state(mt6789_mt6366_pinctrl.pinctrl, "extamp-pulllow");
		if (IS_ERR(mt6789_mt6366_pinctrl.extamp_pulllow)) {
			dev_err(&pdev->dev, "unable to lookup pinctrl state, err: %ld\n", PTR_ERR(mt6789_mt6366_pinctrl.extamp_pulllow));
			mt6789_mt6366_pinctrl.extamp_pulllow = NULL;
		}
	}
